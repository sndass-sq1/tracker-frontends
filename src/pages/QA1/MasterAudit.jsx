...
const MasterAudit = ({ module, from }) => {
  changeTabTitle("MasterAudit");
  ...
  const getMasterAudit = async () => {
    try {
      ...
      const response = await apiClient.get(`quality-analyses/master`, {
        params: { ...params },
      });
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      apiClient.post(`quality-analyses/import/master`, values, {
        componentName: "addMasterAudit",
      }),
  });
  ...
  const sampleTemplate = async () => {
    try {
      const response = await apiClient.get(`quality-analyses/master-audit`, {
        responseType: "blob",
      });
  ...
  const handleStore = async () => {
    try {
      ...
      queryClient.invalidateQueries(["getMasterAudit"]);
      queryClient.invalidateQueries(["getPreview"]);
    } catch (error) {
  ...
  return (
    <div className="container-fluid overflow-y-auto">
      ...
      >
        ...
        <Tab eventKey="AddMasterAudit" title="Add Master Audit">
        ...
        <Tab eventKey="MasterAudit" title="Master Audit">
            ...
            <div className="tableparent">
              ...
              <div className="qaaudit-body px-3 pb-3">
                <ReactTable
                  data={data}
                  columns={tableColumns}
                  apiEndPoint={"quality-analyses/master"}
                  queryKey={"getMasterAudit"}
                  search={search}
                  setSearch={setSearch}
                  page={page}
                  setPage={setPage}
