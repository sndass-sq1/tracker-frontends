...
const ConflictColumnsHumana = ({
  ...
  let columns = [
    {
      accessorKey: "auditor_chart.chart_uid",
      header: "File Name",
      editable: true,
      enableSorting: false,
      cell: ({ row }) => {
        const chartid = row.original.auditor_chart.chart_uid;
        return <div className="chart-id">{chartid || "NA"}</div>;
      },
    },
      ...
      cell: ({ row }) => {
        ...
        if (auth.user.role === "coder") {
          isResolved =
            ...
            feedbackList.every((item) => item.coder_status !== null);
        } else if (auth.user.role === "auditor") {
          isResolved =
            feedbackList.length > 0 &&
            feedbackList.every((item) => item.auditor_status !== null);
        } else {
          isResolved =
            feedbackList.length > 0 &&
            feedbackList.every(
...
export default ConflictColumnsHumana;
