...
const AddAudit = () => {
  ...
  const [total_icd, set_total_icd] = useState("");
  ...
  // Calculation for Chart QA Score Starts (old calculation)
  // useEffect(() => {
  //   if (
  ...
  // Calculation for Chart QA Score Starts(error percentage)
  useEffect(() => {
    if (
      formik.values.dx_codes_error ||
      formik.values.admin_error ||
      ...
      let finalValue = dividedValue * 100;
  ...
  // Calculation for DX QA Score Starts(chart quality)
  useEffect(() => {
    if (
      formik.values.codes_deleted ||
      formik.values.original_code_found ||
  ...
  const fetchChartDetails = async (chartId) => {
    ...
    try {
      ...
      if (status === "success" && chartData) {
        ...
        toast.error(message);
        formik.setErrors({ chart_id: message });
      } else if (message) {
        toast.error(message);
        formik.setErrors({ chart_id: message });
      }
  ...
  // const fetchChartDetails = async (chartId) => {
  //   if (!chartId) {
  //     formik.setErrors({ chart_id: "Chart Id is required." });
  //     return;
  //   }
  ...
  //       setChartDetails(chartData);
  //       setSelectedAuditMode("default_audit");
  //       // set_total_icd(chartData.icd)
  //       formik.setFieldValue("primary_chart_id", chartData.id);
  //       formik.setErrors({});
  //       formik.setTouched({}, false);
  //       toast.success(
  //         response.data.message || "Chart data loaded successfully."
  ...
  const SelectAuditTypeOptionList = [
    ...
    { label: "Traditional Lookback", value: "traditional_lookback" },
              ...
              <div className="d-flex">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
                      fetchChartDetails(formik.values.chart_id.trim());
                      setCurrentChartId(formik.values.chart_id.trim());
                    }
                  }}
                  onPaste={(e) => {
                    e.preventDefault();
                    ...
                    formik.setFieldTouched("chart_id", true);

                    formik.validateField("chart_id");
                  }}
                />
                <button
